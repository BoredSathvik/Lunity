...
struct GuiData {
	...
	auto __thiscall displayClientMessage(nuv::string message) -> void {
		if(holder_displayClientMessage == 0) {
			holder_displayClientMessage = Mem::FindSig("48 89 ?? ?? ?? 48 89 ?? ?? ?? 55 57 41 ?? 41 ?? 41 ?? 48 8D ?? ?? ?? 48 81 EC ?? ?? ?? ?? 48 8B ?? ?? ?? ?? ?? 48 33 ?? 48 89 ?? ?? 4C 8B ?? 45 33 ?? 4C 89 ?? ?? ?? 48 C7 44 24 58 ?? ?? ?? ?? 4C 89 ?? ?? ?? 44 88 ?? ?? ?? 4C 89 ?? ?? ?? 48 C7 44 24 38 ?? ?? ?? ?? 4C 89 ?? ?? ?? 44 88 ?? ?? ?? 4C 89 ?? ?? ?? 48 C7 44 24 78 ?? ?? ?? ?? 4C 89 ?? ?? ?? 44 88 ?? ?? ?? C7 45 80 ?? ?? ?? ?? 0F 10 ?? ?? ?? 0F 11 ?? ?? 4C 89 ?? ?? 48 C7 45 A0 ?? ?? ?? ?? 4C 89 ?? ?? 4C 89 ?? ?? 4C 89 ?? ?? 48 8D ?? ?? E8 ?? ?? ?? ?? 90 4C 89 ?? ?? 4C 89 ?? ?? 4C 89 ?? ?? 48 8D ?? ?? ?? ?? ?? 48 8D ?? ?? E8 ?? ?? ?? ?? 90 F3 0F ?? ?? ?? ?? ?? ?? F3 0F ?? ?? ?? 66 44");
		}
		if(holder_displayClientMessage == 0){
